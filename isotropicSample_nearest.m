function isoStack = isotropicSample_nearest(stack, resXY, resZ, reduceRatio)
...
stack = imresize(stack, [round(reduceRatio*size(stack,1)),round(reduceRatio*size(stack,2))],'nearest');
...
if xy_z_ratio > 1
    if ndims(stack)==4
        ...
        tempStack = imresize(zxyStack, [newZnum, SR], 'nearest');
