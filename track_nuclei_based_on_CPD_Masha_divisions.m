...
%     cpd_plot_iter(ptCloud1, ptCloud2Loc);
    ...
    if length(nn_three(:,1))~=length(unique(nn_three(:,1))) % Reject duplicate nearest neighbors
        ...
        for lvd=1:size(dup,1)
            ...
            from = dup(lvd).ind;
            to = dup(lvd).val;
            to_cell = center_point_for_each_label1(to,:);

            if (dup(lvd).length==2)
                %check if 2 matches are likely to be daughters
                from_cell_1 = center_point_for_each_label2(from(1),:);
                from_cell_2 = center_point_for_each_label2(from(2),:);
